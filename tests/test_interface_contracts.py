...
class InterfaceContractTests(unittest.TestCase):
    ...
    def test_final_step_bypasses_case_memory_review(self) -> None:
        class _Message:
            content = '{"final_answer":"Jane Doe"}'
            tool_calls = None
            reasoning_content = ""
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            orch.config.mock_llm = False
            orch.client = _Client()
            result = orch.run_case(TaskCase(index=0, instruction="Who?", task_id="final_review"))
            self.assertEqual(result["pred"], "jane doe")
            rows = [
                json.loads(line)
                for line in Path(result["trajectory_path"]).read_text(encoding="utf-8").splitlines()
    ...
    def test_failed_case_gets_five_model_attempts_then_empty_pred(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result["api_calls"], 17)
            self.assertEqual(client.chat.completions.calls, 17)
            self.assertEqual(result["answer_repair"]["attempts"], 2)
            self.assertIsNotNone(result["forced_answer"])
            self.assertEqual(result["forced_answer"]["attempts"], 10)
    ...
    def test_search_chain_memory_keeps_structured_query_strategy(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            memory = SkepticalMemoryManager(str(root / "memory.json"), max_rules=10)
            update = memory.auto_dream_deduplication(
                "[When an image question has visible text that can anchor the subject] -> Action: promote a visible-text plus object-type query before broad attribute searches is Necessary to achieve Goal G (Confidence: should)",
                base_keywords=["visual", "search_chain"],
                memory_type="query_strategy",
                task_type="visual_qa",
                episode_id="case_1",
                extra_fields={
                    "title": "Promote visible text query first",
                    "lesson_type": "visual_query_bridge",
                    "trigger_pattern": "Visual question includes visible text that can identify the subject.",
                    "trigger_keywords": ["visible text", "object type"],
                    "query_templates": ["<visible text> <object type>"],
                    "prioritize_keywords": ["visible text", "object type"],
                    "fact_slots": ["visual anchor", "requested field"],
                    "tool_sequence": ["search_image", "search_text"],
                    "correct_strategy": [
                        "Start from image candidates, then search visible text with the object type."
                    ],
                    "avoid": ["Do not search only the requested field without the visual anchor."],
                },
            )
            self.assertEqual(update["operation"], "ADD")
            stored = memory.memory_db[0]
            self.assertEqual(stored["memory_type"], "query_strategy")
            self.assertIn("<visible text> <object type>", stored["query_templates"])
            self.assertIn("visible text", stored["prioritize_keywords"])
            prompt_view = memory.format_memories_for_prompt([stored])[0]
            self.assertIn("Query Strategy", prompt_view)
            self.assertIn("Early query template", prompt_view)
            self.assertIn("Tool order: search_image -> search_text", prompt_view)

    def test_case_memory_parses_json_protocol_for_32b_reviews(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            manager = CaseMemoryManager(
                ...
                api_key="EMPTY",
                max_model_billion=32,
                case_log_path=str(root / "case_memory.jsonl"),
                strategy_path=str(root / "strategy_memory.jsonl"),
            )
            candidate_retry = manager._parse_candidate_answer_review_text(
                json.dumps(
                    {
                        "accept": False,
                    }
            ...
            self.assertFalse(candidate_retry["accept"])
            candidate_accept = manager._parse_candidate_answer_review_text(
                json.dumps(
                    {
                        "accept": True,
                    }
            ...
            self.assertTrue(candidate_accept["accept"])
            retry_context = manager._parse_retry_context_text(
                json.dumps({"context": "search_text 返回了两个候选页面摘要；browser_navigate 对其中一个页面返回了正文预览。"})
            )
            self.assertIn("候选页面摘要", retry_context["context"])

            strategy = manager._parse_strategy_write_text(
                json.dumps(
                    {
                        "should_write": True,
                        "strategy": "When a task asks for <rare entity> <requested field>, search that exact pair early.",
                    }
                )
            )
            self.assertTrue(strategy["should_write"])
            candidate = manager._candidate_from_model_data("case_1", "multi_hop", strategy)
            self.assertEqual(candidate["strategy_type"], "search_strategy")
            self.assertIn("<requested field>", candidate["text_block"])

            selected = manager._parse_strategy_selection_text(
                json.dumps(
                    {
                        "selected_strategy_ids": ["strategy_a", "strategy_b"],
                        "reason": "matching evidence plan",
                    }
                )
            )
            self.assertEqual(selected["selected_strategy_ids"], ["strategy_a", "strategy_b"])

    def test_cognitive_compiler_parses_json_protocols(self) -> None:
        compiler = CognitiveCompiler(enable_model=False)
        reflection = compiler._parse_reflection_text(
            json.dumps(
                {
                    "failure_type": "wrong_requested_field",
                    "root_cause": "The final turn answered an intermediate entity.",
                    "correct_strategy": ["Resolve the intermediate entity, then query the requested field."],
                    "memory_worthy": True,
                    "clin_rule": "[When a task is multi-hop] -> Action: resolve the intermediate entity before the final requested field is Necessary to achieve Goal G (Confidence: should)",
                    "confidence": "should",
                }
            )
        )
        self.assertEqual(reflection["failure_type"], "wrong_requested_field")
        self.assertTrue(reflection["memory_worthy"])

        memory_update = compiler._parse_memory_update_text(
            json.dumps(
                {
                    "operation": "EDIT",
                    "target_id": "rule_1",
                    "rule": "[When visual text identifies the subject] -> Action: search visible text with object type is Necessary to achieve Goal G (Confidence: should)",
                    "reason": "merge similar visual query rules",
                }
            )
        )
        self.assertEqual(memory_update["operation"], "EDIT")
        self.assertEqual(memory_update["target_id"], "rule_1")

        search_chain = compiler._parse_search_chain_text(
            json.dumps(
                {
                    "useful": True,
                    "strategy": "图像题包含可见文字时，把 <visible text> 与 <object type> 作为早期检索锚点。",
                    "reason": "可见文字查询出现得太晚。",
                    "confidence": "should",
                }
            )
        )
        self.assertTrue(search_chain["useful"])
        self.assertIn("<visible text>", search_chain["strategy"])
