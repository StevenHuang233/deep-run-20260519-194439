...
def _parse_text_verdict(text: str) -> dict[str, Any]:
    first = ""
    reason = ""
    verdict = ""
    score: int | None = None
    for line in (text or "").splitlines():
        ...
        if not stripped:
            continue
        if not first:
            first = stripped.upper()
        field = re.match(r"^(VERDICT|SCORE|REASON)\s*:\s*(.*)$", stripped, flags=re.I)
        if not field:
            continue
        key = field.group(1).upper()
        value = field.group(2).strip()
        if key == "VERDICT":
            verdict = value.upper()
        elif key == "SCORE":
            try:
                score = 1 if int(value) == 1 else 0
            except ValueError:
                score = None
        elif key == "REASON":
            reason = value
    signal = verdict or first
    if "INCORRECT" in signal or "错误" in signal or "不正确" in signal:
        return {"score": 0, "verdict": "incorrect", "reason": reason}
    if "CORRECT" in signal or "正确" in signal:
        return {"score": 1, "verdict": "correct", "reason": reason}
    if score is not None:
        return {"score": score, "verdict": "correct" if score else "incorrect", "reason": reason}
    raise ValueError("judge text response must include VERDICT: CORRECT/INCORRECT")
...
class LLMJudge:
    ...
    def score_row(self, row: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            data = _extract_json(content) if "{" in content else _parse_text_verdict(content)
            ...
            verdict = "correct" if score else "incorrect"
            reason = f"judge_parse_failed:{type(exc).__name__}; fallback_exact_match={score}"
        result = self._result(row, score, verdict, reason, attempts)
        if reason.startswith("judge_parse_failed"):
