...
class HarnessOrchestrator:
    ...
    def _run_forced_evidence_answer(
        ...
        max_attempts = max(1, int(self.config.final_answer_max_attempts))
        traj.write_event(
            "forced_answer_start",
            {
                "reason": self._answer_repair_reason(previous_answer),
        ...
        for attempt_no in range(1, max_attempts + 1):
            assistant_step = user_step + attempt_no * 2 - 1
            ...
            if attempt_no < max_attempts:
                ...
                traj.write(
                    ...
                    (
                        ...
                        "不要输出 unknown、unable、cannot、insufficient、工具失败、解释、Markdown 或证据。"
                        "只返回 JSON: {\"final_answer\":\"答案\"}。"
                    ),
                    step_id=retry_step,
                )
                steps_done = retry_step
        ...
        *,
        ...
        traj.write_event(
            "case_memory_candidate_answer_review",
            {
                "issue_type": issue_type,
                "review": review,
                "candidate_answer_preview": self._compact_text(bad_answer, 500),
            },
            step_id=step_id,
        )
        if not bool(review.get("accept")):
