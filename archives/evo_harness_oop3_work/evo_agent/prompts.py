...
# Purpose: 32B only reviews when 9B stops searching and emits a candidate/no-answer; outputs an extractable ACCEPT/RETRY decision plus answer-free memory.
CANDIDATE_ANSWER_REVIEW_PROMPT = """你是候选答案审核器。

模型已经停止检索并输出了一段候选内容，可能是最终答案，也可能是拒答、格式错误、伪工具调用、过长解释或答错字段。
你会看到当前题目、候选输出、已有历史记忆/策略摘要和本 case 的检索记录。

你的任务：
- 判断模型当前输出是否可以作为最终答案提交。
- 如果可以提交，返回 accept=true。
- 如果不能提交，返回 accept=false。

硬性约束：
- 严禁输出、暗示、改写、猜测或替换最终答案；不要给候选答案。
- 只能判断候选输出是否被 observation 支持、是否回答了题目要求字段、是否格式可提交。
- 如果候选内容是工具调用文本、unknown/无法确定、过长解释、答了中间实体、或缺失题目所问字段，返回 accept=false。
- 如果 observation 不足以判断候选答案是否可靠，返回 accept=false。
...
SHORT_ANSWER_REPAIR_PROMPT = (
    ...
    "只返回题目要求的最短答案字段；不要输出 <answer> 标签、解释、证据列表、unknown、unable、cannot、insufficient 或工具失败文本。"
)
...
def build_forced_evidence_answer_prompt(*, task: str, failure_reason: str, evidence: str) -> str:
    return (
        ...
        "如果证据不完整，也必须选择当前证据中最可能的简短答案，不能返回空或拒答。\n"
        "只返回题目要求的最短答案字段；不要输出 <answer> 标签、Markdown、解释、证据列表、unknown、unable、cannot、insufficient 或工具失败文本。\n\n"
        f"Task:\n{task}\n\n"
...
FORCE_ANSWER_PROMPT = (
    ...
    "不要输出推理过程、证据列表、来源说明、Markdown、unknown、unable/cannot/无法/信息不足/search timeout/tool failed。"
    "只返回题目要求的最短答案字段。如果证据不完整，也必须给出最可能的简短答案。"
)
...
def build_answer_issue_retry_prompt(
    *,
    ...
    return (
        ...
        "如果继续检索，避免重复同一个搜索 query、重复访问同一个 URL。"
