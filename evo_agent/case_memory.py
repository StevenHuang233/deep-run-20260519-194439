...
class CaseMemoryManager:
    ...
    def review_candidate_answer(
        ...
        *,
        ...
        """Review only candidate text compliance; never sees evidence or edits answers."""
        if not self.enabled:
            return {
                "accept": True,
                "issue_type": "none",
    ...
    def select_strategies(
        ...
        *,
        ...
        if not self.enable_model:
            return {"selected": [], "source": "model_disabled"}
        try:
            compact_candidates = [
                {
            ...
            return {
                "selected": [],
                "source": "model_error_no_fallback",
                "error": f"{type(exc).__name__}: {exc}",
                "model": self.model_name,
            }
    ...
    def maybe_write_strategy(
        ...
        *,
        ...
        if not self.enable_model:
            return {"operation": "IGNORE", "reason": "model_disabled_no_heuristic_fallback"}
        try:
            content = self._chat_text(
            ...
            return {
                "operation": "IGNORE",
                "reason": "model_error_no_heuristic_fallback",
                "error": f"{type(exc).__name__}: {exc}",
                "model": self.model_name,
            }
    ...
    def _parse_retry_context_text(self, text: str) -> dict[str, Any]:
        data = self._try_extract_json(text)
        if data:
            return {"context": self._sanitize_retry_context(data.get("context") or "")}
        fields = self._parse_field_block(text, multiline_keys={"CONTEXT"})
        return {"context": self._sanitize_retry_context(fields.get("CONTEXT") or text.strip())}

    def _sanitize_retry_context(self, text: Any) -> str:
        context = _compact_text(text or "", 1200).strip()
        if not context:
            return ""
        if context.startswith("{") and '"context"' in context:
            data = self._try_extract_json(context)
            if data:
                context = _compact_text(data.get("context") or "", 1200).strip()
        context = re.sub(r'^\s*["{,]*\s*context\s*["\']?\s*:\s*["\']?', "", context, flags=re.I).strip()
    ...
    def format_records_for_retry_prompt(self, records: list[RetrievalRecord]) -> str:
        if not records:
            return "当前没有检索观察。"
        chunks: list[str] = []
        for record in records[-self.max_records_prompt :]:
            if record.ok:
                query = _compact_text(record.query or "none", 180)
                summary = _compact_text(record.summary, 500) or "无摘要"
                chunks.append(
                    f"- step {record.step_id} {record.tool_name} query={query} "
                    f"返回 {record.result_count} 条；摘要：{summary}"
                )
            else:
                error = _compact_text(record.error, 260) or "unknown_error"
                query = _compact_text(record.query or "none", 180)
                chunks.append(
                    f"- step {record.step_id} {record.tool_name} query={query} 调用失败；错误：{error}"
                )
        return "\n".join(chunks)
